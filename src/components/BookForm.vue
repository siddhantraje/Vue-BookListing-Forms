<template>
  <form v-on:submit.prevent="bookSubmit(bookData)" class="" action="#" method="post">
    ...
    <div>
        <label for="borrowed">borrowed<input v-model="bookData.ownership" type="radio" value="borrowed"></label>
    </div>
    <div>
        <label for="bought">bought<input v-model="bookData.ownership" type="radio" value="bought"></label>
    </div>
